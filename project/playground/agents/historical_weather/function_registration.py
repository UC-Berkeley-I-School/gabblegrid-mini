# File: agents/historical_weather/function_registration.py
from typing import Annotated
import numpy as np  # Already added
import pandas as pd  # Already added
import torch  # Add this import
from sklearn.metrics import confusion_matrix  # Add this import
from .data_preparation import prepare_data_for_model  # Already added
from ...utils.inference import load_model  # Already added
import os
...
def register_historical_weather_functions(agents):
    ...
    def get_historical_weather_data(
        ...
        data_dir = '/home/ubuntu/efs-w210-capstone-ebs/00.GabbleGrid/02.Local_Data_Files'  # Define data_dir within the function
        save_dir = '/home/ubuntu/efs-w210-capstone-ebs/00.GabbleGrid/project/admin/01.Templates'
        file_prefix = "06.20240714_062624_non_overlap_full_test"  # Define file_prefix within the function
        ...
        try:
            ...
            ################################ Changes to fix index issue ######################################################################

            # Calculate the maximum possible num_tests based on X_test size
            max_possible_num_tests = (len(X_test) * num_records_per_test - (start_seq_num - original_df[original_df['Train_Test'] == 'Test']['Seq_Num'].min())) // num_records_per_test
            
            # Use the minimum of calculated num_tests and max_possible_num_tests
            num_tests = min(num_tests, max_possible_num_tests)
            
            print(f"Final num_tests: {num_tests}")
            ...
            metrics_data = {
                # 'Experiment': [experiment_timestamp] * num_tests,
                'Experiment': [experiment_id] * num_tests,
    ...
    def plot_historical_weather_data(
        ...
        image_dir: Annotated[str, "The directory where the plot image should be saved"]
    ) -> dict:
