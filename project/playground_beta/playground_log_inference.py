...
import autogen
from .beta_utils.autogen_setup import TrackableUserProxyAgent, TrackableAssistantAgent  # Updated import
from .beta_utils.inference import send_log_data_and_get_model_results  # Updated import
from .beta_utils.plotting import create_interaction_diagram  # Updated import
...
def display_log_results(results, chat_messages):
    st.markdown("<h3 style='color: tomato;'>All Content is generated by teams of Agents working together on a task</h3>", unsafe_allow_html=True)
    st.markdown("<h4 style='color: grey;'>In this example, we will see 3 teams in actions starting with </h4>", unsafe_allow_html=True)
    st.markdown("<h5 style='color: teal;'>Team 1: Generate Email with key Log Metrics Only </h5>", unsafe_allow_html=True)
