...
def home():
    return render_template('20240718_claude_index_v1.20.html')
...
def handle_inference(data):
    print("Received start_inference event with data:", data)
    start_time = data['start_time']
    num_tests = int(data['num_tests'])

    # Emit an update to show that the server received the request
    emit('update', {'content': f"Starting inference with start_time: {start_time}, num_tests: {num_tests}"})
    ...
    def collect_transcript(message):
        if isinstance(message, dict):
            ...
            emit('update', {'speaker': speaker, 'content': content})
        else:
    ...
    if results is None:
        emit('error', {'message': "Failed to get results from the model"})
