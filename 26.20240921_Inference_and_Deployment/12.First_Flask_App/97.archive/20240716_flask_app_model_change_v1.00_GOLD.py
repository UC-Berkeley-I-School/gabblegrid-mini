...
# Set up logging
logging.basicConfig(filename='/home/ubuntu/efs-w210-capstone-ebs/06.Inference_and_Deployment/12.First_Flask_App/flask.log', level=logging.DEBUG)
...
@log_data_retriever.register_for_llm(description="Run inference on test data and evaluate model performance")
def send_log_data_and_get_model_results(start_time: str, num_tests: int) -> Union[dict, str]:
    ...
    for i in range(num_tests):
        ...
        tracking_data.append([
            ...
            str(original_df[original_df['Seq_Num'] == source_seq_start]['time_start'].values[0])  # Convert timestamp to string
        ])
...
def run():
    ...
    app.logger.debug(f"Received start_time: {start_time}, num_tests: {num_tests}")

    # Create the group chat and run the task
    ...
    with autogen.cache.Cache.disk() as cache:
        # start the conversation
        res = user_proxy.initiate_chat(
            manager, message=task, summary_method="reflection_with_llm", cache=cache, llm_config={}
        )
...
def predict():
    try:
        ...
        app.logger.debug(f"X_test shape before slicing: {X_test.shape}")

        # Ensure X_test is 3-dimensional (batch_size, sequence_length, feature_size)
        if len(X_test.shape) != 3:
            raise ValueError("X_test should be a 3-dimensional array")

        X_test = X_test[:, :, 1:]  # Drop the first column
        app.logger.debug(f"X_test shape after slicing: {X_test.shape}")
