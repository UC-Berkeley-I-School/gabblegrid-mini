...
def home():
    if request.method == 'POST':
        ...
        def collect_transcript(message):
            ...
            if isinstance(message, dict):
                speaker = message.get('name', 'user_proxy')  # Default to user_proxy if name is not present
                content = message.get('content', 'No content')
                
                if speaker != last_speaker:
                    if conversation_transcript:  # Add separator only if it's not the first message
                        conversation_transcript.append("--------------------------------------------------------------------------------")
                    conversation_transcript.append(f"Next speaker: {speaker}")
                
                conversation_transcript.append(f"{speaker} (to chat_manager):")
                conversation_transcript.append(content.strip())  # Remove leading/trailing whitespace
        ...
        # Extract results and collect full conversation transcript
        results = None
    ...
    return render_template('20240718_claude_index_v1.10.html')
