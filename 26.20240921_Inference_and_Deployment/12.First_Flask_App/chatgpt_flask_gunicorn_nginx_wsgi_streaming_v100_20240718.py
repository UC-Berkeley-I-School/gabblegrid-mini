...
class TransformerModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout=0.5):
        ...
        self.transformer = nn.Transformer(hidden_size, nhead=4, num_encoder_layers=num_layers, num_decoder_layers=num_layers, dropout=dropout, batch_first=True)
...
# Autogen setup
llm_config = {
    "config_list": [{"model": "gpt-4o", "api_key": "sk-proj-iQtcgUJOOf4n53Bs6uyqT3BlbkFJnEIqUeEwXjbjVMcDVqiz"}],
}
...
log_data_writer = autogen.AssistantAgent(
    ...
    summarize the content in an email with a subject header and body. Please keep the email concise and focus on the analysis of the confusion matrix.
    Specifically, since this is a log anomaly detection task, please try to analyze the recall and precision of Class 1 (i.e., Alert).
    """,
    llm_config=llm_config,
)
...
def send_log_data_and_get_model_results(start_time: str, num_tests: int) -> Union[dict, str]:
    ...
    socketio.emit('log_update', {'message': f"Loading test data from {data_dir}"})
    ...
    if filtered_df.empty:
        ...
        if not same_date_df.empty:
            ...
            socketio.emit('log_update', {'message': f"Error: start_time {start_time} not found in the dataset. Available times on {start_date} are: {available_times}"})
        ...
        socketio.emit('log_update', {'message': f"Error: start_time {start_time} not found in the dataset. The nearest available time is: {nearest_time}"})
    ...
    socketio.emit('log_update', {'message': 'Sending data to prediction endpoint'})
    response = requests.post('http://127.0.0.1:8000/predict', json=data)
    if response.status_code != 200:
        socketio.emit('log_update', {'message': f"Failed to get predictions: Status Code {response.status_code}"})
    ...
    socketio.emit('log_update', {'message': 'Processing predictions'})
    tracking_data = []
    for i in range(num_tests):
        ...
        socketio.emit('log_update', {'message': f"Processed prediction {i+1}/{num_tests}"})
    ...
    socketio.emit('log_update', {'message': 'Inference completed successfully!'})
    socketio.emit('metrics_update', {'metrics': results['metrics']})
    socketio.emit('confusion_matrix_update', {'confusion_matrix': results['confusion_matrix']})
...
def home():
    if request.method == 'POST':
        ...
        def collect_transcript(message):
            nonlocal last_speaker

            if isinstance(message, dict):
                speaker = message.get('name', 'user_proxy')
                content = message.get('content', 'No content')

                if speaker != last_speaker:
                    if conversation_transcript:
                        conversation_transcript.append("--------------------------------------------------------------------------------")
                    conversation_transcript.append(f"Next speaker: {speaker}")

                conversation_transcript.append(f"{speaker} (to chat_manager):")
                conversation_transcript.append(content.strip())

                last_speaker = speaker
            else:
                content = str(message).strip()
                if content:
                    conversation_transcript.append(content)
        ...
        socketio.emit('log_update', {'message': 'Inference completed successfully!'})
        socketio.emit('metrics_update', {'metrics': metrics})
        socketio.emit('confusion_matrix_update', {'confusion_matrix': confusion_matrix})

        return render_template('20240718_claude_results_v1.20.html',
                               metrics=metrics,
                               confusion_matrix=confusion_matrix,
                               output_file=output_file,
                               conversation_transcript=conversation_transcript)

    return render_template('20240718_chatgpt_results_v1.10.html')
