...
def main():
    ...
    if st.button('Run Inference'):
        ...
        # Display conversation transcript dynamically
        placeholder = st.empty()
        while True:
            chat_messages = st.session_state['chat_messages']
            chat_content = []
            for sender, receiver, message in chat_messages:
                if isinstance(message, dict) and 'content' in message:
                    content = message['content']
                    chat_content.append(f"**{sender} (to {receiver}):**\n\n```\n{content}\n```\n\n---")
                else:
                    chat_content.append(f"**{sender} (to {receiver}):**\n\n{message}\n\n---")
        ...
        # Display final conversation transcript
        st.subheader('Final Conversation Transcript:')
