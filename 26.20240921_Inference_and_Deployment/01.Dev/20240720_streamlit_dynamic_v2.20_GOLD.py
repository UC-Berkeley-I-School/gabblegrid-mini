...
def send_log_data_and_get_model_results(start_time: str, num_tests: int, status_update_callback=None) -> Union[dict, str]:
    if status_update_callback:
        status_update_callback("Initializing model loading process...", 15)
    print("Loading the model...")
    model = load_model()
    if status_update_callback:
        status_update_callback("Model loaded successfully.", 20)
    print("Model loaded.")
    ...
    if status_update_callback:
        status_update_callback("Preparing to load test data...", 25)
    print("Loading test data...")
    X_test = np.load(f"{data_dir}/{file_prefix}_X_test.npy")
    y_test = np.load(f"{data_dir}/{file_prefix}_y_test.npy")
    if status_update_callback:
        status_update_callback("Test data loaded successfully.", 30)

    if status_update_callback:
        status_update_callback("Loading original input data...", 35)
    ...
    if status_update_callback:
        status_update_callback("Original input data loaded successfully.", 40)

    if status_update_callback:
        status_update_callback(f"Filtering test dataset based on start time: {start_time}", 45)
    ...
    if filtered_df.empty:
        if status_update_callback:
            status_update_callback("Start time not found. Searching for alternatives...", 50)
    ...
    if status_update_callback:
        status_update_callback("Preparing data for inference...", 55)
    ...
    if status_update_callback:
        status_update_callback("Starting prediction process...", 60)
    ...
    if status_update_callback:
        status_update_callback("Predictions completed.", 65)

    if status_update_callback:
        status_update_callback("Generating tracking data...", 70)
    tracking_data = []
    ...
    if status_update_callback:
        status_update_callback("Saving tracking data to CSV...", 75)
    ...
    if status_update_callback:
        status_update_callback("Tracking data saved successfully.", 80)

    if status_update_callback:
        status_update_callback("Calculating performance metrics...", 85)
    ...
    if status_update_callback:
        status_update_callback("Preparing final results...", 90)
    results = {
        "metrics": {
            "Accuracy": accuracy,
    ...
    if status_update_callback:
        status_update_callback("Results generated successfully.", 95)
...
def main():
    ...
    if st.button('Run Inference'):
        ...
        status_placeholder.text("Initializing inference process...")
        progress_bar.progress(10)

        st.markdown("""
            <h2 style='color: #4169E1; font-size: 24px;'>GabbleGrid: Proactive Cloud Service Reliability</h2>
        ...
        status_placeholder.text("Initializing agents...")
        ...
        if not st.session_state.chat_initiated:
            status_placeholder.text("Starting inference process...")
            ...
            async def run_chat():
                ...
                status_placeholder.text("Initiating inference...")
        ...
        if results is None:
            ...
            status_placeholder.text("Generating final report...")
            ...
            status_placeholder.text("Process completed successfully!")
