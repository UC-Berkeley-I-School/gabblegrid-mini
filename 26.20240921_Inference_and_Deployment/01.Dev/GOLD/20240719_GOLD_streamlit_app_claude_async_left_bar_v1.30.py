...
class TrackableAssistantAgent(autogen.AssistantAgent):
    ...
    def _process_received_message(self, message, sender, silent):
        message_id = f"{sender.name}:{message.get('content', '')[:50]}"
        if message_id not in self.displayed_messages:
                ...
                if isinstance(message, dict):
                    ...
                    try:
                        ...
                        if isinstance(json_content, dict):
                            ...
                            st.markdown(f"**{sender.name}**: {content}")
                    except json.JSONDecodeError:
                        st.markdown(f"**{sender.name}**: {content}")
                else:
                    st.markdown(f"**{sender.name}**: {message}")
        ...
        for message in user_proxy.chat_messages[manager]:
            if isinstance(message, dict):
                ...
                if message_id not in seen_messages:
                    ...
                    conversation_transcript.append(f"**{speaker}**:\n```\n{content}\n```")
