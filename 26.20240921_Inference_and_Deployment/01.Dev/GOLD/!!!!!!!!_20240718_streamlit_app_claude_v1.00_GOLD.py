...
if st.button('Run Inference'):
    ...
    def collect_transcript(message, last_speaker=[None]):
        if isinstance(message, dict):
            speaker = message.get('name', 'user_proxy')
            content = message.get('content', 'No content')
            
            if speaker != last_speaker[0]:
    ...
    if results is None:
        ...
        st.text_area('Transcript', value='\n'.join(conversation_transcript), height=300)

# Run the Streamlit app
# This is all you need to run a Streamlit app
