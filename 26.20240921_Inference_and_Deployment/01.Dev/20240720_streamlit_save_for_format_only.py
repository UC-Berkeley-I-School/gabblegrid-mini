...
class TrackableAgent(autogen.ConversableAgent):
    ...
    def _process_received_message(self, message, sender, silent):
        ...
        if unique_message not in st.session_state['seen_messages']:
            ...
            st.session_state['chat_messages'].append((sender.name, self.name, message))
...
# Background function to run the chat and capture messages
async def run_chat_task(user_proxy, manager, start_time, num_tests):
    task = f"""
    ...
    with Cache.disk() as cache:
        await user_proxy.a_initiate_chat(manager, message=task, summary_method="reflection_with_llm", cache=cache)
...
def format_results(results):
    if isinstance(results, str):
        return results  # Return error messages as is

    formatted_output = ""
    
    # Metrics
    formatted_output += "Model Performance Metrics:\n"
    ...
    formatted_output += f"  [[{conf_matrix[0][0]}, {conf_matrix[0][1]}]\n"
    formatted_output += f"   [{conf_matrix[1][0]}, {conf_matrix[1][1]}]]\n"
    ...
    if st.button('Run Inference'):
        ...
        if not st.session_state.get("chat_initiated", False):
            with st.spinner("Running inference..."):
                asyncio.run(run_chat_task(user_proxy, manager, start_time, num_tests))
                st.session_state.chat_initiated = True
            st.success("Inference complete!")

        # Display the chat messages with formatted results
        for sender, receiver, message in st.session_state['chat_messages']:
            with st.expander(f"Message from {sender} to {receiver}"):
                if isinstance(message, dict) and 'content' in message:
                    ...
                    if isinstance(content, str):
                        try:
                            ...
                            st.text(format_results(json_content))
                        except json.JSONDecodeError:
                            # If it's not JSON, display as is
                            st.write(content)
                    elif isinstance(content, dict):
                        st.text(format_results(content))
                else:
                    st.write(message)
