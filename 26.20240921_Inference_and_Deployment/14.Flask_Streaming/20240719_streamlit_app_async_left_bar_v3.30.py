...
def create_sequence_diagram(transcript_content):
    G = nx.DiGraph()
    lines = transcript_content.strip().split("\n")
    for i in range(len(lines)):
        if " (to " in lines[i]:
            source = lines[i].split(" ")[0]
            target = lines[i].split("(to ")[1].split("):")[0]
            G.add_edge(source, target)

    pos = nx.spring_layout(G)
    plt.figure(figsize=(14, 10))
    nx.draw(G, pos, with_labels=True, node_color='lightgreen', node_size=3000, edge_color='black', linewidths=1, font_size=12, arrowsize=15)
    sequence_diagram_path = '/home/ubuntu/efs-w210-capstone-ebs/06.Inference_and_Deployment/09.Images_Docs_Other_Dynamic_Content/sequence_diagram.png'
    plt.savefig(sequence_diagram_path)
        ...
        if results is None:
            ...
            for message in user_proxy.chat_messages[manager]:
                if isinstance(message, dict):
                    ...
                    transcript_content += f"{speaker} (to chat_manager):\n\n{content}\n\n"
            ...
            sequence_diagram_path = create_sequence_diagram(transcript_content)
            st.image(sequence_diagram_path)
