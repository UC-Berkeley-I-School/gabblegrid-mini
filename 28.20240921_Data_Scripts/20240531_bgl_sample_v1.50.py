...
# Define the output directory for sampled data
sampled_output_dir = os.path.join(base_dir, '11.Data/01.BGL/04.Sampled_Parsed_CFDR')
os.makedirs(sampled_output_dir, exist_ok=True)

# Define the file path
parsed_file_path = os.path.join(base_dir, '11.Data/01.BGL/03.Parsed_CFDR/parsed.csv')
...
def do_sample(source, train_ratio, sample_ratio=1.0, sorting="random", tw=None):
    header = True
    sequences_extracted = {}
    tw_groups = {} # Only used for time-window based grouping
    tw_labels = {} # Only used for time-window based grouping
    labels = {}
    with open(parsed_file_path) as extracted, open(os.path.join(sampled_output_dir, 'bgl_train.csv'), 'w+') as train, open(os.path.join(sampled_output_dir, 'bgl_test_normal.csv'), 'w+') as test_norm, open(os.path.join(sampled_output_dir, 'bgl_test_abnormal.csv'), 'w+') as test_abnormal:
        cnt = 0
        ...
        for label, seq_id_dict in sequences_extracted.items():
            if label == 'Normal':
                ...
                with open(os.path.join(sampled_output_dir, 'bgl_test_abnormal_' + label + '.csv'), 'w+') as test_label:
                    for seq_id, event_list in seq_id_dict.items():
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", default="bgl_cfdr", help="path to input files", type=str, choices=['adfa_verazuo', 'hdfs_xu', 'hdfs_loghub', 'bgl_loghub', 'bgl_cfdr', 'openstack_loghub', 'openstack_parisakalaki', 'hadoop_loghub', 'thunderbird_cfdr', 'awsctd_djpasco'])
    ...
    do_sample(source=base_dir + '/11.Data/01.BGL/03.Parsed_CFDR', train_ratio=args.train_ratio, sample_ratio=args.sample_ratio, sorting=args.sorting, tw=args.time_window)
